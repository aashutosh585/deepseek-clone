...
const ChatLabel = ({openMenu, setOpenMenu, id, name}) => {
  ...
  return (
    <div onClick={selectChat} className="flex items-center justify-between p-2 text-white/80 hover:bg-white/10 rounded-lg text-sm group cursor-pointer">
      ...
      <div onClick={e=>{e.stopPropagation();setOpenMenu({id: id, open: !openMenu.open})}} className="group relative flex items-center justify-center h-6 w-6 aspect-square hover:bg-black/60 rounded-lg">
        ...
        <div className={`absolute ${openMenu.id === id && openMenu.open ? 'block' : 'hidden'} -right-36 top-6 bg-gray-700 rounded-xl w-max p-2`}>
          ...
          <div onClick={deleteHandler} className="flex items-center gap-3 hover:bg-red-400/20 px-3 py-2 rounded-lg">
            <Image src={assets.delete_icon} alt="" className="w-5" />
